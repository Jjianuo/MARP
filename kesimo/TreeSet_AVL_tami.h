...
public:
   ...
   T const& kesimo(int k) const {
       if (k < 1 || k > nelems) throw std::out_of_range("??");
       return kesim(raiz, k);
   }
protected:

    T kesim(Link a, int k) const {
        if (k < a->tam_i) { //el elemento esta a la izquierda
            return kesim(a->iz, k);
        }
        else if (k > a->tam_i) { //el elemento esta a la derecha
            return kesim(a->dr, k - a->tam_i);
        }
        else { //se ha encontrado la posicion k
            return a->elem;
        }
    }
   ...
   bool inserta(T const& e, Link & a) {
      ...
      if (a == nullptr) { // se inserta el nuevo elemento e
         ...
         if (crece) { //si se inserta se suma
             ++a->tam_i;
             reequilibraDer(a);
         }
      } else if (menor(a->elem, e)) {
